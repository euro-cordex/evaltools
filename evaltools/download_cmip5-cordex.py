...
def datasetid_2_dataframe(dataset_id):
    ...
    columns = ['project', 'product', 'domain', 
               'institute', 'driving_model', 'experiment', 
               'ensemble', 'rcm_name', 'rcm_version', 
...
def download_datasetid_ESGF(dataset_id, nodeURL, dest):
    ...
    if not nc_files:
        ...
        subprocess.check_output(['bash', "{}".format(script_path), '-s'], cwd=target_dir) 
...
def main():
    ...
    dreq = pd.read_csv("https://raw.githubusercontent.com/euro-cordex/joint-evaluation/refs/heads/main/dreq_EUR_joint_evaluation.csv")
    variables = dreq[dreq['priority'].str.contains('overview', case=False, na=False)]['out_name'].values.tolist()
    frequencies = dreq[dreq['priority'].str.contains('overview', case=False, na=False)]['frequency'].values.tolist()
    ...
    for var, freq in product(variables, frequencies):
        ...
        if freq == 'mon':
            ...
            extra = rcm_list_day[~rcm_list_day.drop(columns=facets_excluded).apply(tuple, axis=1).isin(
                rcm_list.drop(columns=facets_excluded).apply(tuple, axis=1))]        
            # Concatenate results
    ...
    for indx, row in df_rcms.iterrows():
        ...
        var_not_found = [var for var in variables if var not in result['variable'].values]
